...
public class MysuccessionEvaluator implements SuccessionEvaluator<PetriNet, Transition>
{
  private MyPetriSuccessionEvaluator petriSuccessionEvaluator;
  private boolean transientAnalysis;
  
  public MysuccessionEvaluator(boolean transientAnalysis, MarkingUpdater tokensRemover, MarkingUpdater tokensAdder, boolean distinctNewlyEnablingConditions, OmegaBigDecimal tauAgeLimit)
  {
  ...
  public StoSuccession computeSuccession(PetriNet petriNet, StoState state, Transition fired)
  {
    ...
    else
    {
      ...
      firedPriority = (Priority)fired.getFeature(Priority.class);
					...
					//这一段是计算有没有可能有和t1同时发生的，例如t3.这样就改变了概率
...
@Override
public Succession computeSuccession(PetriNet arg0, State arg1, Transition arg2) {
