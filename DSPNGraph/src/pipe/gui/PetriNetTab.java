...
public class PetriNetTab extends JLayeredPane implements Observer, Printable
{
    ...
    public class MouseHandler extends MouseInputAdapter
    {
        ...
        private ConnectableView newTransition(Point p, int type)
        {
            p = adjustPoint(p, _petriNetTab.getZoom());

           
            pn = new TransitionView((double) Grid.getModifiedX(p.x), (double) Grid.getModifiedY(p.y),type);
            //我觉得这一行不能实现控制图形是因为图形paintComponent()已经画完了
            // ((TransitionView) pn).setType(0);
        ...
        public void mousePressed(MouseEvent e)
        {
            ...
            if(SwingUtilities.isLeftMouseButton(e))
            {
                ...
                switch(mode)
                {
                    ...
                    case Constants.EXPTRANS:
                    case Constants.DETTANS:	
                    //case Constants.UNITRANS:
                    //case Constants.ERLTRANS:
                    	int type = 0;
                    	if(mode == Constants.EXPTRANS)
                    		type = 1;
                    	else if(mode == Constants.DETTANS)
                    		type = 2;
                    	/*else if(mode == Constants.UNITRANS)
                    		type = 3;
                    	else if(mode == Constants.ERLTRANS)
                    		type = 4;
//                    	else if(mode == Constants.EXPOLTRANS)
//                    		type = 5;*/
                        
                        pto = newTransition(e.getPoint(), type);
                        _petriNetTab.getHistoryManager().addNewEdit(new AddPetriNetObject(pto, _petriNetTab, _model));
                        if(e.isControlDown())
                        {
...
//                            _petriNetTab._wasNewPertiNetComponentCreated = true;
//                            timed = e.isAltDown();
//                            if(applicationModel.getOldMode() == Constants.EXPTRANS)
//                                timed = !timed;
